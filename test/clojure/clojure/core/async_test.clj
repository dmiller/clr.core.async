(ns clojure.core.async-test
  ...
  (:import [System.Threading Thread]))                 ;;; DM:Added
...
(deftest take!-on-caller?
  (is (apply not= (let [starting-thread (Thread/CurrentThread)                                           ;;; Thread/currentThread
                      test-channel (chan nil)
                      read-promise (promise)]
                  (take! test-channel (fn [_] (deliver read-promise (Thread/CurrentThread))) true)       ;;; Thread/currentThread
                  (>!! test-channel :foo)
  ...
  (is (apply = (let [starting-thread (Thread/CurrentThread)                                              ;;; Thread/currentThread
                      test-channel (chan nil)
                      read-promise (promise)]
  ...
  (is (apply not= (let [starting-thread (Thread/CurrentThread)                                           ;;; Thread/currentThread
                      ...
                      read-promise (promise)]
                  (put! test-channel :foo (constantly nil))
                  (take! test-channel (fn [_] (deliver read-promise (Thread/CurrentThread))) false)      ;;; Thread/currentThread
                  [starting-thread @read-promise]))
...
(deftest put!-on-caller?
  (is (apply = (let [starting-thread (Thread/CurrentThread)                                              ;;; Thread/currentThread
                 ...
                 (put! test-channel :foo #(deliver write-promise (Thread/CurrentThread)) true)           ;;; Thread/currentThread
  ...
  (is (apply not= (let [starting-thread (Thread/CurrentThread)                                           ;;; Thread/currentThread
                        ...
                        write-promise (promise)]
                    (take! test-channel (fn [_] nil))
                    (put! test-channel :foo #(deliver write-promise (Thread/CurrentThread)) false)       ;;; Thread/currentThread
                    [starting-thread @write-promise]))
...
(deftest ops-tests
 ...
 (testing "pub-sub"
  ...
  (testing "partition-by"
    (is (= [["a" "b"] [1 :two 3] ["c"]]
           (<!! (a/into [] (a/partition-by string? (a/to-chan ["a" "b" 1 :two 3 "c"]))))))))